...
(function(jQuery) {
	...
	jQuery.autogrow.fn.extend({
		...
		init: function() {
			...
			this.textarea.bind('focusunction() { self.startExpand() } ).bind('blur', function() { self.stopExpand });
			this.checkExpand();
		},
		...
		checkExpand: function() {
			...
			if (this.dummy == null)
			{
				...
				this.dummy.css({
					'font-size'  : this.textarea.css('font-size					'font-family': this.textarea.css('font-family					'width'      : this.textarea.css('width					'padding'    : this.textarea.css('padding					'line-height': this.textarea.css('line-height					'overflow-x' : 'hidden',
					'display'    : 'none',
					'position'   : 'absolute',
					'top'        : 0,
					'left'       : '-9999px'
